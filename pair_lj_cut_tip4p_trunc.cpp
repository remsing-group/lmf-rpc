...
/* Modified for LMF Gaussian-truncation by R. Remsing, October 2023 */


#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "pair_lj_cut_tip4p_trunc.h"
#include "atom.h"
#include "force.h"
#include "neighbor.h"
...
PairLJCutTIP4PTrunc::PairLJCutTIP4PTrunc(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  writedata = 1;
...
PairLJCutTIP4PTrunc::~PairLJCutTIP4PTrunc()
{
  if (allocated) {
...
void PairLJCutTIP4PTrunc::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul;
  double r,rsq,r2inv,r6inv,forcecoul,forcelj,factor_lj,factor_coul;
  double rinv, grij, expm2,prefactor,t,erfc;
...
void PairLJCutTIP4PTrunc::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutTIP4PTrunc::settings(int narg, char **arg)
{
...
void PairLJCutTIP4PTrunc::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 5)
...
void PairLJCutTIP4PTrunc::init_style()
{
...
double PairLJCutTIP4PTrunc::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutTIP4PTrunc::write_restart(FILE *fp)
{
...
void PairLJCutTIP4PTrunc::read_restart(FILE *fp)
{
...
void PairLJCutTIP4PTrunc::write_restart_settings(FILE *fp)
{
...
void PairLJCutTIP4PTrunc::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutTIP4PTrunc::write_data(FILE *fp)
{
...
void PairLJCutTIP4PTrunc::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairLJCutTIP4PTrunc::compute_newsite(double *xO,  double *xH1,
...
void *PairLJCutTIP4PTrunc::extract(const char *str, int &dim)
{
  dim = 0;
...
double PairLJCutTIP4PTrunc::memory_usage()
{
